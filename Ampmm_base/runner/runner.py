...
from .api import batch_processor
...
class Runner(BaseRunner):
    ...
    def build_dataloaders(self, cfg):
        ...
        self.train_dataloader = build_data_loader(
            self.train_dataset,cfg.data['train'].batch_per_gpu)
        self.val_dataloader = build_data_loader(
            self.val_dataset,cfg.data['val'].batch_per_gpu)
    
        self.test_dataset = build_dataset(cfg,'test')

        self.test_dataloader = build_data_loader(
            self.test_dataset,cfg.data['test'].batch_per_gpu)
    ...
    def register_evaluation_hooks(self):
        ...
        """
        if self.cfg.task == 'rank': # rank
            self.register_hook(RankEvalHook(),10)
        elif self.cfg.task == 'reg': # reg
            self.register_hook(RegEvalHook(),10)

    def register_test_hooks(self):
        """
        Register hooks for analysing results of test.
        """
        self.register_hook(AnalyseHook(self.cfg.work_dir),10)
