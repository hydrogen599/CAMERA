...
from esm_project import Alphabet
...
class EmbeddingProcessor:
    ...
    def __init__(self) -> None:
        self.pretrain_model, _ = esm.pretrained.esm2_t33_650M_UR50D()
        alphabet = Alphabet.from_architecture("roberta_large")
        self.batch_converter = alphabet.get_batch_converter()
        self.pretrain_model = self.pretrain_model.to(device)
        self.pretrain_model.eval()  # disables dropout for deterministic results
        self.all_seqs = []
    ...
    def generate_embeddings(self, outdir, mode='all', fname='esm_embeddings.h5'):
        ...
        assert (mode == 'all') or (mode == 'pooling') or (mode == 'cls_token') or (mode == 'residue_token')
        os.makedirs(outdir, exist_ok=True)
        self.max_len = max(len(seq) for seq in self.all_seqs)
        max_len = 64
        if mode == 'all' or mode == 'residue_token':
            max_len = self.max_len
            print("Max length: {} {}".format(self.max_len, max_len))
        ...
        with h5py.File(os.path.join(outdir,fname), 'w') as hf:
            for seq in tqdm(self.all_seqs):
                ...
                elif mode == 'residue_token':
                    embedding = token_representations[:,1:max_len-1,:].squeeze(0)  # cls token
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bac', default='bac11') # E.coli
    ...
    datasets_list = [os.path.join(datasets_dir,i) for i in ['train.csv', 'val.csv', 'test.csv']]
    processor.get_seqs_from_datasets(datasets_list)
    processor.generate_embeddings('', mode=mode, fname=fname)